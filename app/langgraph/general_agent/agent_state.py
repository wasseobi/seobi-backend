...
@dataclass
class AgentState:
    """General Agent의 상태를 나타내는 타입 (MCP + 직접 구현 도구 통합)"""
    ...
    mcp_tools: Optional[List] = None  # MCP 도구 목록
    direct_tools: Optional[List] = None  # 직접 구현 도구 목록
    ...
    def set_tools(self, mcp_tools: List = None, direct_tools: List = None) -> None:
        """도구 목록 설정"""
        if mcp_tools:
