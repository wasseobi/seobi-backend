...
class ReportService:
    ...
    def get_weekly_daily_reports(self, user_id: UUID, tz: timezone) -> List[Dict]:
        """주간 데일리 리포트 조회"""
        week_start, week_end = TimeUtils.get_week_range(tz)
        return self.report_dao.get_reports_by_date_range(
    ...
    def generate_report(self, user_id, tz_str, report_type):
        ...
        if report_type == 'daily':
            report_data = self.generator.format_report_content(user_id, tz, report_type='daily')
        elif report_type == 'weekly':
            report_data = self.generator.format_report_content(user_id, tz, report_type='weekly')
